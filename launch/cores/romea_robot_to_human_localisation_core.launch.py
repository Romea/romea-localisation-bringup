...
def get_leader_motion_noise_std(context):
    return float(LaunchConfiguration("leader_motion_noise_std").perform(context))


def get_use_constraints(context):
    return bool(LaunchConfiguration("use_constraints").perform(context))
...
def launch_setup(context, *args, **kwargs):
    ...
    leader_motion_noise_std = get_leader_motion_noise_std(context)
    use_constraints = get_use_constraints(context)
    ...
    configuration_file_path = "/tmp/" + robot_namespace + "_robot_to_human_localisation_core.yaml"
    save_configuration(configuration, configuration_file_path)

    core = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_package_share_directory("romea_robot_to_human_localisation_core")
            + "/launch/"
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument("launch_file", default_value="robot_to_robot_localisation.launch.py")
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument("maximal_dead_recknoning_elapsed_time", default_value="1.")
    )
    ...
    declared_arguments.append(DeclareLaunchArgument("use_constraints"))
